...
def rnn(
    ...
    # Get time steps from the first input tensor
    ...
    if mask is None and input_length is not None:
        ...
        if input_len_ov.get_partial_shape().rank.get_length() == 1:
            ...
            # input_len: [1, batch]
            input_len_casted = ov_opset.convert(input_len_ov, Type.i32).output(0)
            input_len_expanded = ov_opset.unsqueeze(
                input_len_casted, ov_opset.constant([0], dtype=Type.i32).output(0)
            ).output(0)
    ...
    if mask is not None:
        ...
        # Ensure mask is [time, batch, 1] for correct broadcasting in loop
        # Current shape is likely [time, batch] or [time, batch, 1]
    ...
    # Determine output structure and initial value for last_output
    def _slice_at_0(x):
        x_ov = get_ov_output(x)
    ...
    for inp in flattened_inputs:
        ...
        if pshape.rank.is_static:
            ...
            new_shape = [-1] * (pshape.rank.get_length() - 1) if pshape.rank.is_static else None
    ...
    for st in flattened_states:
        ...
        param = ov_opset.parameter(st_ov.get_partial_shape(), st_ov.get_element_type())
        merged_states_params.append(param)
        params.append(param)

    # 4. Last Output States
    last_output_params = []
    for lo in last_output_states:
        lo_ov = get_ov_output(lo)
        param = ov_opset.parameter(lo_ov.get_partial_shape(), lo_ov.get_element_type())
    ...
    for c in flattened_constants:
        ...
        param = ov_opset.parameter(c_ov.get_partial_shape(), c_ov.get_element_type())
        constants_params.append(param)
        params.append(param)

    # --- Body Logic ---

    sliced_inputs_t = [OpenVINOKerasTensor(p.output(0)) for p in sliced_inputs_params]
    merged_states_t = [OpenVINOKerasTensor(p.output(0)) for p in merged_states_params]
    ...
    if mask is not None:
        ...
        for i, (new_st, old_st) in enumerate(zip(flat_step_new_states, merged_states_t)):
        ...
        for i, (new_out, old_last_out) in enumerate(zip(flat_step_output, last_output_params)):
            new_out_ov = get_ov_output(new_out)
            old_last_out_ov = old_last_out.output(0)

            last_out_res = ov_opset.select(mask_t, new_out_ov, old_last_out_ov).output(0)
            final_last_output_list.append(last_out_res)

            if zero_output_for_mask:
                zero = ov_opset.broadcast(
                    ov_opset.constant(0, new_out_ov.get_element_type()).output(0),
                    ov_opset.shape_of(new_out_ov, Type.i32).output(0)
                ).output(0)
                seq_out_res = ov_opset.select(mask_t, new_out_ov, zero).output(0)
            else:
    ...
    results = [cond_const] + final_states_list + final_last_output_list + final_output_list
    ...
    for param, init in zip(merged_states_params, flattened_states):
        loop.set_merged_input(param, get_ov_output(init), results[current_res_idx])
        current_res_idx += 1
    ...
    for _ in final_output_list:
        out = loop.get_concatenated_slices(results[current_res_idx], 0, 1, 1, -1, 0)
